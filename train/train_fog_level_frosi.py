...
file_train = open(cp.get(section, 'acc_fog_level_train_frosi'), 'w')
file_valid = open(cp.get(section, 'acc_fog_level_valid_frosi'), 'w')
...
trainset = MyDataSet(
    root=cp.get(section, 'root_frosi'),
    datatxt=cp.get(section, 'train_frosi'),
    tranform=simple_transform
)
validset = MyDataSet(
    root=cp.get(section, 'root_frosi'),
    datatxt=cp.get(section, 'valid_frosi'),
    tranform=simple_transform
)
...
for i in range(0, epoch):
    ...
    for _, x1, labels_cls, labels_reg in train_loader1:
        #data input
    ...
    loss_val = 0.0
    correct_val = 0
    for _, inputs1,  labels1_cls, labels1_reg in valid_loader:
    ...
    if i%5 == 0:
        path = '/home/liqiang/Documents/parameters/fog_level_single_frosi/'+'epoch_{}'.format(i) + '.pth'
