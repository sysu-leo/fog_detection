...
from my_model.TaskModule import FogLevel_Classify, VisDistance_Estimation, FogLevel_Classify_FC, Visibility_Estimation_FC, Visibility_Estimation_FC_na, Visibility_Estimation_FC_Nocas
...
class Multi_Task_2_na(nn.Module) :
    def __init__(self):
        super(Multi_Task_2_na, self).__init__()
        self.softmax = nn.Softmax(dim=1)
        self.g_feature = g_exactor(in_chanel=4, out_chanel=1024)
        self.d_feature = d_exactor(input_channel =  3, out_channel = 256)
        self.task1 = FogLevel_Classify_FC()
...
class Multi_Task_2_Nocas(nn.Module) :
    def __init__(self):
        super(Multi_Task_2_Nocas, self).__init__()
        self.softmax = nn.Softmax(dim=1)
        self.g_feature = g_exactor(in_chanel=4, out_chanel=1024)
        self.d_feature = d_exactor(input_channel =  3, out_channel = 256)
        self.task1 = FogLevel_Classify_FC()
        self.task2 = Visibility_Estimation_FC_Nocas()
