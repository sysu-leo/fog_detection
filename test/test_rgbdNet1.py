...
from my_model.EnvNet1 import EnvNet1
...
testset = MyDataSet(
    root='../daqiao/',
    file_rgbd = 'test_daqiao',
    file_dark = 'test_daqiao_dark',
    file_slice='test_daqiao_slice',
    datatxt='../Dataset/daqiao.txt',
    tranform=simple_transform,
    is_slice=False
)
...
# load weight
weight_path = '../Parameters/model_rgbd1/epoch{}.pth'.format(epoch)
model = testrgbdNet1()
...
# train
correct = 0
res_path = '../test_result/rgbdNet1_daiqiao_epoch{}.txt'.format(epoch)
...
for img_name, x1,labels in test_loader:
    x1 = x1.to(device)
    output = model(x1)
    print(count)
    count += 1
